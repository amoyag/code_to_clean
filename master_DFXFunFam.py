...
Function:  master 
    ...
    small molecule binding to FunFam domains.
...
def getHumanTargets(config):
    ...
    return targets
#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
def getmd5(target,cathconfig):
        global md5
        querytarget = "'" + target +"'"
        #get the md5 from the UniProt id
        query = """select u.sequence_md5 from CATH_DATA.UNIPROT_DESCRIPTION u where u.uniprot_acc = %s """ %(querytarget)
        connection = cx_Oracle.connect(cathconfig['username'], cathconfig['pword'], cx_Oracle.makedsn(cathconfig['host'], 1521, cathconfig['sid']))
        #connection = cx_Oracle.connect("biomap_reader", "4biomapuse", cx_Oracle.makedsn("sinatra", 1521, "biomapwh"))
        cursor = cx_Oracle.Cursor(connection)
        cursor.execute(query)
...
def getFunFams(targets, cathconfig, config):
    ...
    for target in targets:
        querytarget = "'" + target +"'"
        md5seq = getmd5(target,cathconfig)
        if not md5seq:continue 
        domains = []
        query = """select sequence_md5, funfam from gene3d_12.mfam_hits_old where sequence_md5 = %s """ %(md5seq)
        # For FunfHmmer FunFams:
        ...
        for domain in domains:
            ...
            pfamDict[target]['start'].append(int(0)) # Not intereted in start/end of domains
...
def master(): 
    ...
    ## Get Functional Families as domains
    pfamDict = getFunFams(targets, cathconfig, config) 
   
    ## Export the PfamDict (FunFam) as a (mysql) table.
    ...
    ## Export the mapping as a txt file
    exportMaps(propDict)
